...
parser.add_argument('--base_RF_loss_weight', type=float, default=1.)  #weight for base random field loss, i.e. f-E[f]
...
#loader to produce labeled CIFAR10 data
loader_lab = torch.utils.data.DataLoader(
...
def cycle(iterable):
    while True:
        for x in iterable:
            yield x
loader_lab = iter(cycle(loader_lab))
#loader to produce unlabeled CIFAR10 data
loader_unl = torch.utils.data.DataLoader(
    datasets.CIFAR10(args.data_root,train=True,download=True,
    transform=transforms.Compose([transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])),
    batch_size=args.batch_size, shuffle=True, num_workers=1, pin_memory=True,drop_last=True)

loader_unl = iter(cycle(loader_unl))

h_dim = 128

#initialize random field and generator,using ResNet
import model_supervised
RF = model_supervised.Discriminator().cuda()
generator = model_supervised.Generator(h_dim).cuda()
print(RF)
sys.stdout.flush()

# because the spectral normalization module creates parameters that don't require gradients (u and v), we don't want to 
# optimize these using sgd. We only let the optimizer operate on parameters that _do_ require gradients
optim_RF = optim.Adam(filter(lambda p: p.requires_grad, RF.parameters()), lr=args.lrd, betas=(0.5,0.999))
optim_G  = optim.Adam(filter(lambda p: p.requires_grad, generator.parameters()), lr=args.lrg, betas=(0.5,0.999))

def log_sum_exp(logits, mask=None, inf=1e7):
    if mask is not None:
        logits = logits * mask - inf * (1.0 - mask)
        max_logits = logits.max(1,keepdim=True)[0]
        return ((logits - max_logits.expand_as(logits)).exp() * mask).sum(1).log() + max_logits.squeeze()
    else:
        max_logits = logits.max(1,keepdim=True)[0]
        return ((logits - max_logits.expand_as(logits)).exp()).sum(1).log() + max_logits.squeeze()
...
def revision(gen_images):
    ...
    for i in range(args.L):
        loss_revision = torch.mean(log_sum_exp(RF(x_revised)))
...
def train():
        ...
        lab_images, lab_labels = next(loader_lab)
        lab_images,lab_labels= lab_images.cuda(),lab_labels.cuda()
        unl_images, _ = next(loader_unl)
        ...
        u_unl=log_sum_exp(RF(unl_images))
        u_revised=log_sum_exp(RF(x_revised))
        unl_loss = (torch.mean(u_revised) - torch.mean(u_unl)) * args.base_RF_loss_weight + args.potential_control_weight * torch.mean(u_unl ** 2)
        lab_logit = RF(lab_images)
        lab_loss = F.cross_entropy(lab_logit, lab_labels)
        loss_RF = lab_loss + unl_loss
        #update random field
        optim_RF.zero_grad()
        loss_RF.backward()
        optim_RF.step()
        ...
        h = Variable(torch.randn(args.batch_size, h_dim).cuda())
        gen_images = generator(h)
        g_loss=-torch.mean(log_sum_exp(RF(gen_images)))
        # update generator
        optim_G.zero_grad()
        g_loss.backward()
        optim_G.step()
...
def generate():
    ...
    plt.savefig('cifar_image/NRF_supervised_{}.png'.format(args.suffix), bbox_inches='tight')
    plt.close(fig)
