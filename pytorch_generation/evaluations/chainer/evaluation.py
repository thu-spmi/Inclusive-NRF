...
def load_inception_model(path=None):
    path = path if path is not None else "%s/../../../data/inception_model"% os.path.dirname(__file__)
    model = Inception()
...
def calc_inception(ims, path=None, splits=1):
...
def calc_FID(ims, stat_file, path=None):
