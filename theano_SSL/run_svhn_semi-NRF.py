...
parser.add_argument('--count', type=int, default=100)   #how much data one class
...
parser.add_argument('--max_e', default=400 ,type=int)   #max number of epochs
...
parser.add_argument('--data_dir', default='../data' ,type=str)    #file name to load trained model
...
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 64, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_1'),name='d_w1'))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 64, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_2'),name='d_w2'))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 64, (3,3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_3'),name='d_w3'))
layers.append(ll.dropout_channels(layers[-1], p=0.5))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 128, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_4'),name='d_w4'))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 128, (3,3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_5'),name='d_w5'))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 128, (3,3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_6'),name='d_w6'))
layers.append(ll.dropout_channels(layers[-1], p=0.5))
layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(layers[-1], 128, (3,3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_7'),name='d_w7'))
layers.append(nn.weight_norm(ll.NINLayer(layers[-1], num_units=128, W=Normal(0.05), nonlinearity=nn.lrelu,name='d_8'),name='d_w8'))
...
for epoch in range(args.max_e):
    ...
    if epoch==0:
        ...
        if args.load:
            load_weights('svhn_model/svhn_jrf_'+args.load+'.npy', layers + gen_layers)
    ...
    if (epoch + 1) % 50 == 0:
        import os
        if not os.path.exists('svhn_model'):
            os.mkdir('svhn_model')
        params = ll.get_all_params(layers + gen_layers)
        save_weights('svhn_model/semi_nrf_data%d_ep%d.npy' % (args.seed_data, epoch + 1), params)
    if loss_unl<-100:
        break
