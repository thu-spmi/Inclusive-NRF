...
revision=th.function(inputs=[gen_dat], outputs=x_revised)
...
np.random.shuffle(trainx_copy)

for it in range(args.max_iterations):

    start_id = (it*args.batch_size)%trainx.shape[0]
    # for CD, samples are starting from dataset
    gen_dat = trainx_copy[start_id:start_id+args.batch_size]
    x_revised = revision(gen_dat)
    ...
    if (it+1)%10000==0:
        ...
        for j in range(100):
            inds = np.random.permutation(len(trainx))[:100]
            gen_dat = trainx_copy[inds]
            x_revised = revision(gen_dat)
        ...
        if (it + 1) % 100000 == 0:
            ...
            inds = np.random.permutation(len(trainx))[:1000]
            gen_dat = trainx_copy[inds]
            x_revised = revision(gen_dat)
            #plot generated and revised data, and potentials
            datautil.plot_data(gen_dat,'CD_gen_it%d_%s'%(it+1,args.suffix))
            datautil.plot_data(x_revised,'CD_rev_it%d_%s'%(it+1,args.suffix))
            datautil.plot_ux(potential_function,'CD_ux_it%d_%s'%(it+1,args.suffix))
