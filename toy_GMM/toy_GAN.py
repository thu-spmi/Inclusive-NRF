...
layers.append(nn.DenseLayer(layers[-1], num_units=1, nonlinearity=T.nnet.sigmoid,train_scale=True,name='d_3' ))
...
loss_D = -T.mean(T.log(x_out))-T.mean(T.log(1.0-sam_out))
...
for it in range(args.max_iterations):
    ...
    if (it+1)%10000==0:
        ...
        if (it + 1) % 100000 == 0:
            ...
            datautil.plot_data(gen_dat,'gan_it%d_%s'%(it+1,args.suffix))
